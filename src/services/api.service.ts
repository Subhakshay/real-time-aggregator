...
/**
 * Fetches and transforms data from DexScreener
 */
export const fetchFromDexScreener = async (query: string): Promise<Token[]> => {
  try {
    ...
    if (error instanceof Error) {
      ...
      console.error('DexScreener API Error:', 'An unknown error occurred');
    }
    return [];
  }
};
...
/**
 * Fetches and transforms data from Jupiter
 */
export const fetchFromJupiter = async (query: string): Promise<Token[]> => {
  try {
    ...
    if (error instanceof Error) {
      ...
      console.error('Jupiter API Error:', 'An unknown error occurred');
    }
    return [];
  }
};
